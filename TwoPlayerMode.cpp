...
Vector2i TwoPlayerMode :: getCord(Figure A) {
	return Vector2i((int)A.shape.getPosition().x / SIZE, (int)A.shape.getPosition().y / SIZE);
}

void TwoPlayerMode :: PawnLogic(Figure A, vector<Vector2i>& move) {
...
void TwoPlayerMode::KingLogic(Figure A, vector <Vector2i>& move) {
	Vector2i cord = getCord(A);
...
void TwoPlayerMode::KnightLogic(Figure A, vector<Vector2i>& move) {
	Vector2i cord = getCord(A);
...
void TwoPlayerMode::Horizontal(Figure A, vector<Vector2i>& move) {
	Vector2i cord = getCord(A);
...
void TwoPlayerMode::Vertical(Figure A, vector<Vector2i>& move) {
	Vector2i cord = getCord(A);
...
void TwoPlayerMode::Diagonal(Figure A, vector<Vector2i>& move) {
	Vector2i cord = getCord(A);
	if (cord.x < 0 || cord.y < 0) return; 
...
void TwoPlayerMode::drawUndoRedo(RenderWindow& window) {
...
bool TwoPlayerMode::isValidMove(Figure A, Vector2i pos) {
...
void TwoPlayerMode::getValidMove(Figure A, vector<Vector2i>& move) {
	if (A.type == pawn_main) {
		PawnLogic(A, move); 
		return; 
	}
...
bool TwoPlayerMode::TakePiece(Vector2i pos) {
...
void TwoPlayerMode::UpdBoardState() {
...
bool TwoPlayerMode::KingSafe(Vector2i temp) {
	bool ok = 1;
...
void TwoPlayerMode::Castling(vector <Vector2i>& move) {
	if (isWhiteTurn) {
...
void TwoPlayerMode::drawValidMoves(RenderWindow& window, const std::vector<Vector2i>& validmoves) {
	int TILE_SIZE = SIZE;
	int TILE_OFFSET = 24;
...
void TwoPlayerMode::UpdHistoryWhite(Figure A, Vector2i pos) {
...
void TwoPlayerMode::UpdHistoryBlack(Figure A, Vector2i pos) {
	array<int, 4> lastMove;
...
bool TwoPlayerMode::WarningCheck(Figure A, Vector2i pos) {
...
void TwoPlayerMode::drawPawnPromote(RenderWindow& window) {
...
int TwoPlayerMode::PawnPromotion(Event event, RenderWindow& window) {
...
void TwoPlayerMode::UndoRedo(Event event, RenderWindow& window) {
	Vector2i pos = Mouse::getPosition(window);

	if (event.type == Event::MouseButtonPressed && event.mouseButton.button == Mouse::Left && Undo.getGlobalBounds().contains(pos.x, pos.y) && NumberOfMove > 0) {
		UseUndo();
	}

	if (event.type == Event::MouseButtonPressed && event.mouseButton.button == Mouse::Left && Redo.getGlobalBounds().contains(pos.x, pos.y) && NumberOfUndo > 0) {
		UseRedo();
	}

	if (event.type == sf::Event::KeyPressed) {
		if (event.key.code == sf::Keyboard::Left && NumberOfMove > 0) {
			UseUndo();
		}

		if (event.key.code == sf::Keyboard::Right && NumberOfUndo > 0) {
			UseRedo();
		}
	}
}
void TwoPlayerMode::ResetAll() {
	index = -1;
	pawnPromote = pieceSelected = NumberOfMove = NumberOfUndo = 0;
	isWhiteTurn = 1; isGameEnd = 0; 
	validMoves.clear();
...
int TwoPlayerMode::handleEvents(RenderWindow& window) {
	Event event; 
            ...
            if (pieceSelected == 0) {
				...
				if (isWhiteTurn) {
					if (pawnPromote) {
						...
						if (WarningCheck(White[index], target)) sound[2].play();
						TakePiece(target);
					...
					if (isValidMove(White[index], temp)) {
						...
						if (WarningCheck(White[index], temp)) sound[2].play();
						...
						for (int i = 0; i < 12; i++) {
							Vector2f p = Vector2f(temp.x * SIZE + 0.5, temp.y * SIZE + 0.5) - Vector2f(White[index].shape.getPosition());
					...
					if (isValidMove(Black[index], temp)) {
						if (Black[index].type == pawn_main && temp.y == 7) {
							pawnPromote = 1;
							target = temp; 
							continue;
						}
						...
						if (WarningCheck(Black[index], temp)) sound[2].play();
							...
							if (temp.x == cord.x + 2 && temp.y == cord.y) Black[7].shape.setPosition((temp.x - 1) * SIZE, temp.y * SIZE), sound[3].play();
							else if (temp.x == cord.x - 2 && temp.y == cord.y) Black[0].shape.setPosition((temp.x + 1) * SIZE, temp.y * SIZE), sound[3].play();
							else sound[0].play();
						}
						else sound[0].play();
						...
						if (Black[index].played == 0 && index == 4) {
							if (temp.x == cord.x + 2 && temp.y == cord.y) Black[7].shape.setPosition((temp.x - 1) * SIZE, temp.y * SIZE), Black[7].played ++;
							else if (temp.x == cord.x - 2 && temp.y == cord.y) Black[0].shape.setPosition((temp.x + 1) * SIZE, temp.y * SIZE), Black[0].played ++;
						}
...
void TwoPlayerMode::BoardSetUp(string path, char numberBoard) {
	string BoardPath = "images/BoardGame/board";
...
void TwoPlayerMode::draw(sf::RenderWindow& window) {
	window.draw(Board); 
