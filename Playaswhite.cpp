...
bool Playaswhite::WarningCheck(Figure A, Vector2i pos) {
	...
	Vector2i cord = getCord(A);
	if (cord.x < 0 || cord.y < 0 || pos.x < 0 || pos.y < 0) return 0; 
	...
	for (auto move : a) {
		if (BoardState[move.x][move.y] == king_main + 100 || BoardState[move.x][move.y] == king_main + 200) {
			...
			//cout << A.type << "\n";
			//cout << move.x << ' ' << move.y << "\n\n";
			//cout << Occur[move.x][move.y] << ' ' << Occur[pos.x][pos.y] << "\n\n"; 
...
void Playaswhite::drawPawnPromote(RenderWindow& window) {
...
int Playaswhite::PawnPromotion(Event event, RenderWindow& window) {
...
Vector2i Playaswhite::getAImove() {
	Vector2i valid; 
	...
	if (Medium) {
		for (int i = 0; i < 16; i++) {
			Vector2i cord = getCord(White[i]); 
			computer.White[i] = {White[i].type, cord.x, cord.y, White[i].played};
		}

		for (int i = 0; i < 16; i++) {
			Vector2i cord = getCord(Black[i]);
			computer.Black[i] = { Black[i].type, cord.x, cord.y, Black[i].played};
		}

		pair<int,pii> search = computer.decision();
		index = search.first; 

		return Vector2i(search.second.first, search.second.second); 
	}
	...
	while (window.pollEvent(event)) {
		...
		if (isWhiteTurn == 0) {
			...
			if (Black[index].type == pawn_main && AI.y == 7) {
				...
				if (WarningCheck(Black[index], AI)) sound[2].play();
				TakePiece(AI);
			...
			for (int i = 0; i < 10; i++) {
				Vector2f p = Vector2f(AI.x * SIZE + 0.5, AI.y * SIZE + 0.5) - Vector2f(Black[index].shape.getPosition());

				Black[index].shape.move(p.x / 10, p.y / 10);
				window.draw(Board);
				...
				if (isWhiteTurn) {
					...
					if (isValidMove(White[index], temp)) {
						...
						for (int i = 0; i < 10; i++) {
							...
							White[index].shape.move(p.x / 10, p.y / 10);
							window.draw(Board);
